...
4. `pattern.setTransform(new DOMMatrix().rotate(-30))` 旋轉整個 pattern
5. 以 pattern 填滿整張畫布
...
| 項目 | 預設 | 可調 | 理由 |
|---|---|---|---|
| 顏色 | 紅 | 白／黑／紅／靛藍 | 權狀為白底，紅色最醒目且符合「副本」慣例 |
| 透明度 | 25% | 滑桿 8%–60% | 蓋得住，但底下地號、坪數仍可辨讀 |
| 字級 | 圖寬 4% | 滑桿 2%–8% | 一行約 8–12 字寬 |
| 密度 | 倍率 1.0 | 滑桿 0.7–2.5（左密右疏） | 倍率 1.0 時 A4 約 5 行 × 3 列 |
| 角度 | -30° | 不可調 | 45° 過陡，易與文件表格線衝突 |
| 帶今日日期 | 開啟 | 可關 | 標明影本發出時間 |
