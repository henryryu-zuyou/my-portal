...
- 吃照片，也吃 PDF（多頁可自行選要哪幾頁）
- 浮水印無法靠裁切、截圖、局部放大來規避
- 機密照片全程不離開使用者裝置
- 手機與電腦都能用

**非目標（明確不做）**
- 批次多張「照片」處理（使用者評估後排除；PDF 多頁則支援選頁）
- PDF 進、PDF 出（見下方決策）
- 伺服器端影像處理（違背「照片不離開裝置」）
- 輸出 PNG（一律 JPEG）
...
PDF 每頁在瀏覽器光柵化成點陣圖，蓋上浮水印後輸出 JPG。

替代方案是輸出 PDF（用 pdf-lib 疊一層浮水印圖層），保留文字可搜尋、頁數不變。
**否決**：那層浮水印是獨立的 PDF 圖層，隨便一個 PDF 編輯器就能整層刪掉，
防冒用的效果幾乎歸零 —— 而防冒用正是本工具存在的理由。
光柵化後浮水印烙進像素，刪不掉。

代價是輸出不再是 PDF、文字無法選取搜尋。使用者已知悉並選擇此方案。

### PDF：為什麼不需要新依賴

用專案已有的 `unpdf`（內含 pdfjs 5）的 `unpdf/pdfjs` 匯出，在瀏覽器
`getDocument` + `page.render` 到 canvas。PDF 同樣不離開裝置。

過程中曾誤判 `unpdf` 在瀏覽器無法 render 而安裝了 `pdfjs-dist`，後來發現
真正的原因是渲染卡在 `requestAnimationFrame`（見下），`unpdf` 本身沒問題，
故移除該依賴。兩者也不能並存：`GlobalWorkerOptions` 是全域的，
混用會出現 `API version does not match Worker version`。

### PDF：渲染一律用 intent "print"

pdfjs 的 `display` intent 靠 `requestAnimationFrame` 分批繪製。
瀏覽器會凍結背景分頁的 rAF，使用者切走再回來，渲染就永遠停在半路
（`getDocument`、`getOperatorList` 都正常，只有 render 不回來，症狀很像掛死）。

`intent: "print"` 不走 rAF（pdfjs 原始碼：`useRequestAnimationFrame: !intentPrint`），
而本工具要的正是「整頁完整光柵化」，語意本來就相符。

### PDF：輸出解析度低於照片

`PDF_MAX_EDGE = 3000`（照片是 `MAX_EDGE = 4096`）。

PDF 是向量，解析度由我們決定。A4 長邊 3000px 約 255dpi，對文件閱讀已很清楚，
而像素數只有 4096 長邊的一半多 —— 渲染與 JPEG 編碼都快得多。
實測 production build 三頁約 7.9 秒（每頁 2.6 秒）。
...
| 非圖片也非 PDF（Word/Excel 等） | 「只吃圖片和 PDF。Word、Excel 請先轉存成 PDF 或圖片。」 |
| PDF 加密或損壞 | 「這個 PDF 打不開，可能是加密或檔案損壞。」 |
| PDF 一頁都沒選 | 下載鈕停用，顯示「請至少選一頁」 |
...
3. 逐項確認：
   ...
   - PDF：頁面縮圖數等於頁數、預設全選、選頁與下載檔名的頁碼一致
     （只選第 1、3 頁應得到 `-p1-`、`-p3-`，不是流水號 `-p1-`、`-p2-`）
   - PDF：切到某頁再切回第 1 頁，預覽必須跟著重繪
     （曾因切頁 effect 有 `currentPage === 1` 早退條件而顯示錯頁）
   - PDF 與照片互相切換後，選頁 UI 與檔案資訊都要正確重置
