...
export const FAQ_ENTRIES: FaqEntry[] = [
  ...
  },
  {
    ...
    "text": "保守估計是市場行情的 5 到 7 折 ，實際成數要看屋況才能定。 折數太低會踩到一個稅務地雷。 要適用房屋稅 1.5% 的優惠稅率，租金必須「申報租賃所得達當地租金標準」。桃園市的租金標準是房屋評定現值的 22% （104 年 7 月 1 日前竣工、適用舊標準單價的房屋）或 10% （之後竣工、適用新標準單價），兩者都要再加公告土地現值的 1.2%。以現值 80 萬的房子為例：舊標準單價要年租金 176,000 元 以上才算達標，市價月租 2 萬打 7 折是 168,000， 已經低於標準 ；新標準單價的門檻只有 80,000 元起跳，7 折甚至 5 折通常都達得到。沒達標房屋稅會被改按 3.2% 起跳課徵，一年多繳 1 萬 3,600。 影響成數的主要是這幾項： 屋況與屋齡 ——需要整理的程度越低，成數越高 格局與坪數 ——好租的格局（套房、兩房）通常條件較好 地段與交通 ——去化速度直接影響我們願意承擔的風險 是否附傢俱家電 ——附得越齊，成數越高 委託年限 ——期間越長，攤提成本越低，成數越高 我們不會在看屋前給您一個數字，因為那個數字沒有意義。實際看過、拍過、確認過屋況之後，會給您一份書面的租金評估，成數與依據都會寫在上面。 比較的時候記得把稅一起算。 包租的租金收入一樣適用租賃專法的優惠：每屋每月 6,000 元免稅額，超過的部分月租 2 萬以內可扣除 53% 必要費用。但折數壓得太低會低於租金標準、連房屋稅優惠一起丟掉，所以「折數高低」不等於「實拿高低」。"
  },
  {
    "hasCalc": false,
    "id": "q5",
